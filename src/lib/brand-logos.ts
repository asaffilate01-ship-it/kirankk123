import nearcuraLogo from "@/assets/nearcura-logo.png.asset.json";
...
import eastamiraLogo from "@/assets/eastamira-logo.png.asset.json";
import corazoraLogo from "@/assets/corazora-logo.png.asset.json";
import canavelleLogo from "@/assets/canavelle-logo.png.asset.json";
...
import travenexiaLogo from "@/assets/travenexia-logo.png";
...
import merqanoAsset from "@/assets/merqano-logo-v2.png.asset.json";
const merqanoNewLogo = merqanoAsset.url;
...
export const BRAND_LOGOS: Record<string, string> = {
  ...
  eastamira: eastamiraLogo.url,
  corazora: corazoraLogo.url,
  canavelle: canavelleLogo.url,
...
/**
 * Brands whose logo has a language-specific lockup (German vs English slogan).
 * The correct variant is picked from the active site language.
 */
export const BRAND_LOGOS_BY_LANG: Record<string, Record<Lang, string>> = {
  ...
  eventplanruk: { de: eventplanrLogoDe.url, en: eventplanrLogoEn.url },
};

export function brandLogo(id: string, lang: Lang = "en"): string | undefined {
  return BRAND_LOGOS_BY_LANG[id]?.[lang] ?? BRAND_LOGOS[id];
}
