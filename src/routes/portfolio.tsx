...
function PublicBrandCard({ brand, lang }: { brand: Brand; lang: "en" | "de" }) {
  ...
  return (
    <Card className="flex flex-col p-4"><div className="flex items-start gap-3"><BrandLogoBox src={brandLogo(brand.id, lang)} name={brand.name} color={brand.color} /><div className="min-w-0"><h3 className="font-semibold">{brand.name}</h3><p className="mt-0.5 text-xs text-muted-foreground">{t(brand.tagline)}</p><div className="mt-2 flex flex-wrap gap-1"><Badge variant="secondary" className="text-[10px]">{COUNTRIES.find((item) => item.id === countryOf(brand))?.flag} {t(countryLabel(countryOf(brand)))}</Badge><Badge variant="outline" className="text-[10px]">{t(sectorLabel(sectorOf(brand)))}</Badge></div></div></div>
