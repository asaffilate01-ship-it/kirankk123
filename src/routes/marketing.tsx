...
function MarketingPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="sticky top-0 z-30 border-b bg-card/90 backdrop-blur"><div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3"><div className="flex min-w-0 items-center gap-3"><BrandLogo className="h-10 shrink-0 sm:h-14" /><div><h1 className="text-sm font-semibold">{t("Portfolio marketing")}</h1><p className="text-[11px] text-muted-foreground">{t("Private brand marketing plans")}</p></div></div><div className="flex items-center gap-2"><Button asChild variant="outline" size="sm"><Link to="/portfolio"><Store className="mr-1 h-3.5 w-3.5" />{t("Public portfolio")}</Link></Button><LanguageToggle /><Button variant="outline" size="sm" onClick={handleLock}><LogOut className="mr-1 h-3.5 w-3.5" />{t("Lock")}</Button></div></div></header>
